...
1. [Q1. What are LINQ quantifier operations, and what type do they return?](#q1-what-are-linq-quantifier-operations-and-what-type-do-they-return)
2. [Q2. What does `Any()` (parameterless) do, and when should you prefer it over `Count() > 0`?](#q2-what-does-any-parameterless-do-and-when-should-you-prefer-it-over-count-0)
3. [Q3. What does `Any(predicate)` do, and how does its short-circuit behavior work?](#q3-what-does-anypredicate-do-and-how-does-its-short-circuit-behavior-work)
4. [Q4. What does `All(predicate)` do, and what is its short-circuit behavior on failure?](#q4-what-does-allpredicate-do-and-what-is-its-short-circuit-behavior-on-failure)
5. [Q5. What is vacuous truth, and how does it affect `All(predicate)` on an empty sequence?](#q5-what-is-vacuous-truth-and-how-does-it-affect-allpredicate-on-an-empty-sequence)
6. [Q6. What does `Contains(value)` do, and what equality mechanism does it use by default?](#q6-what-does-containsvalue-do-and-what-equality-mechanism-does-it-use-by-default)
7. [Q7. How does `Contains(value, IEqualityComparer<T>)` differ from the default overload?](#q7-how-does-containsvalue-iequalitycomparert-differ-from-the-default-overload)
8. [Q8. Why does `Contains` on a class type without `Equals`/`GetHashCode` often return `false` even when the data matches?](#q8-why-does-contains-on-a-class-type-without-equalsgethashcode-often-return-false-even-when-the-data-matches)
9. [Q9. What does `SequenceEqual` do, and what are the conditions for it to return `true`?](#q9-what-does-sequenceequal-do-and-what-are-the-conditions-for-it-to-return-true)
10. [Q10. How does `SequenceEqual` behave with sequences of different lengths?](#q10-how-does-sequenceequal-behave-with-sequences-of-different-lengths)
11. [Q11. What is the difference between `SequenceEqual` and set equality?](#q11-what-is-the-difference-between-sequenceequal-and-set-equality)
12. [Q12. What happens when you call any quantifier operation on a `null` source?](#q12-what-happens-when-you-call-any-quantifier-operation-on-a-null-source)
13. [Q13. When should you use `Any()` combined with `All(predicate)` rather than `All(predicate)` alone?](#q13-when-should-you-use-any-combined-with-allpredicate-rather-than-allpredicate-alone)
14. [Q14. How does `Any(predicate)` compare to `Where(predicate).Any()` in terms of behavior and performance?](#q14-how-does-anypredicate-compare-to-wherepredicateany-in-terms-of-behavior-and-performance)
15. [Q15. What are the empty-sequence return values for each quantifier operation?](#q15-what-are-the-empty-sequence-return-values-for-each-quantifier-operation)
16. [Q16. How do `readonly record struct` and `record class` types affect `Contains` and `SequenceEqual`?](#q16-how-do-readonly-record-struct-and-record-class-types-affect-contains-and-sequenceequal)
17. [Q17. How do quantifier operations relate to C# query syntax?](#q17-how-do-quantifier-operations-relate-to-c-query-syntax)
18. [Q18. What is the performance difference between `Any(predicate)` and `Count(predicate) > 0`?](#q18-what-is-the-performance-difference-between-anypredicate-and-countpredicate-0)
19. [Q19. What does `All(predicate)` return when called on a brand-new `List<T>` with no elements added? (Gotcha)](#q19-what-does-allpredicate-return-when-called-on-a-brand-new-listt-with-no-elements-added-gotcha)
20. [Q20. Why does `shipmentList.Contains(new OrderLine("WH-4412", ...))` return `false` even though the list has an item with that SKU? (Gotcha)](#q20-why-does-shipmentlistcontainsnew-orderlinewh-4412-return-false-even-though-the-list-has-an-item-with-that-sku-gotcha)
21. [Q21. A developer checks non-empty like this: `var batch = repo.GetLines(); if (batch.Count() > 0 && batch.All(line => line.Quantity > 0))`. What can go wrong with a deferred `IEnumerable<T>` source? (Gotcha)](#q21-a-developer-checks-non-empty-like-this-var-batch-repogetlines-if-batchcount-0-batchallline-linequantity-0-what-can-go-wrong-with-a-deferred-ienumerablet-source-gotcha)
22. [Q22. `["UPS","FEDEX","DHL"].Contains(scannedCode)` rejects a valid carrier whose code arrives from the scanner as `"fedex"`. Why, and how do you fix it? (Gotcha)](#q22-upsfedexdhlcontainsscannedcode-rejects-a-valid-carrier-whose-code-arrives-from-the-scanner-as-fedex-why-and-how-do-you-fix-it-gotcha)
23. [Q23. `SequenceEqual` returns `false` for two lists built from the same database query. What are the two most likely causes? (Gotcha)](#q23-sequenceequal-returns-false-for-two-lists-built-from-the-same-database-query-what-are-the-two-most-likely-causes-gotcha)
24. [Q24. A shipment release service has the following validation. Review the code, identify the defects, and describe the fix. (Scenario)](#q24-a-shipment-release-service-has-the-following-validation-review-the-code-identify-the-defects-and-describe-the-fix-scenario)
25. [Q25. A compliance team requires that every line in a hazmat shipment either carries `IsHazardous = true` OR has a unit price under $100. The developer writes `batch.All(line => line.IsHazardous || line.UnitPrice < 100m)`. A QA test with an empty batch passes. Describe the risk and the correct implementation. (Scenario)](#q25-a-compliance-team-requires-that-every-line-in-a-hazmat-shipment-either-carries-ishazardous-true-or-has-a-unit-price-under-100-the-developer-writes-batchallline-lineishazardous-lineunitprice-100m-a-qa-test-with-an-empty-batch-passes-describe-the-risk-and-the-correct-implementation-scenario)
26. [Q26. An integration test compares two `List<OrderLine>` objects — one loaded from the database and one built in code — using `SequenceEqual`. The test always fails even when the data appears identical in the debugger. Diagnose the cause and describe how to make the test pass without modifying `OrderLine`. (Scenario)](#q26-an-integration-test-compares-two-listorderline-objects-one-loaded-from-the-database-and-one-built-in-code-using-sequenceequal-the-test-always-fails-even-when-the-data-appears-identical-in-the-debugger-diagnose-the-cause-and-describe-how-to-make-the-test-pass-without-modifying-orderline-scenario)
27. [Q27. A product catalog service needs to check if any of a set of incoming SKUs are already on a restricted list. A junior developer writes: `restrictedSkus.Any(r => incomingSkus.Contains(r))`. A senior developer says this has an O(n²) problem. Explain the concern and propose a better approach. (Scenario)](#q27-a-product-catalog-service-needs-to-check-if-any-of-a-set-of-incoming-skus-are-already-on-a-restricted-list-a-junior-developer-writes-restrictedskusanyr-incomingskuscontainsr-a-senior-developer-says-this-has-an-on-problem-explain-the-concern-and-propose-a-better-approach-scenario)
28. [Q28. A dock scanner reconciliation service verifies that the physical scan order matches the printed manifest. The service uses `dockManifest.SequenceEqual(scannerOutput)` and reports a mismatch even though an operator confirms every item was scanned. What are the three most likely explanations, and what diagnostic steps would you take? (Scenario)](#q28-a-dock-scanner-reconciliation-service-verifies-that-the-physical-scan-order-matches-the-printed-manifest-the-service-uses-dockmanifestsequenceequalscanneroutput-and-reports-a-mismatch-even-though-an-operator-confirms-every-item-was-scanned-what-are-the-three-most-likely-explanations-and-what-diagnostic-steps-would-you-take-scenario)
29. [Q29. You are reviewing a feature flag service that checks if the active flags contain all required flags for a feature. The implementation is `requiredFlags.All(f => activeFlags.Contains(f))`. Under what conditions does this silently allow unauthorized feature access, and how would you fix it? (Scenario)](#q29-you-are-reviewing-a-feature-flag-service-that-checks-if-the-active-flags-contain-all-required-flags-for-a-feature-the-implementation-is-requiredflagsallf-activeflagscontainsf-under-what-conditions-does-this-silently-allow-unauthorized-feature-access-and-how-would-you-fix-it-scenario)
30. [Q30. A legacy ETL pipeline uses the following code to determine whether a re-extracted batch matches the previously imported batch. Review the code, identify all defects, provide an Issues table, and give a fix priority list. (Scenario)](#q30-a-legacy-etl-pipeline-uses-the-following-code-to-determine-whether-a-re-extracted-batch-matches-the-previously-imported-batch-review-the-code-identify-all-defects-provide-an-issues-table-and-give-a-fix-priority-list-scenario)
31. [Q31. A shopping cart service must apply a promotional discount when a cart contains at least one item from a specific set of promotional SKUs. The discount should not apply when the cart is empty. Implement `HasPromotionalItem(IReadOnlyList<CartItem> cart, IReadOnlyCollection<string> promoSkus)` using the appropriate LINQ quantifier. Explain every choice. (Scenario)](#q31-a-shopping-cart-service-must-apply-a-promotional-discount-when-a-cart-contains-at-least-one-item-from-a-specific-set-of-promotional-skus-the-discount-should-not-apply-when-the-cart-is-empty-implement-haspromotionalitemireadonlylistcartitem-cart-ireadonlycollectionstring-promoskus-using-the-appropriate-linq-quantifier-explain-every-choice-scenario)
