 ...
 *   11. Runtime bootstrap — EnsureReady()
 */

/*
 * SECTION 11: RUNTIME BOOTSTRAP — EnsureReady()
 *
 * Creates the database and dbo.Products if missing, resets the table when an older
 * schema is detected, and seeds sample rows before the chapter demo runs.
 */
public static class AdoNetTutorialBootstrap
{
    public const string DatabaseName = "AdoNetTutorial";

    public static string ConnectionString { get; } = ConnectionHelper.AdoNetTutorial;
