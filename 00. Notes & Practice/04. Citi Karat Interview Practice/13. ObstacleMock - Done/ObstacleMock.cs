...
namespace DotNetQuestions
{
    ...
    */
    ...
    public class RunCollection
    {
        ...
        {
            return Runs.Count > 0 ? Runs.Where(r => r.Complete).Min(r => r.GetRunTime()) : int.MaxValue;
        }
        ...
        public int BestOfBests()
        {
            ...
            {
                int bestAtObstacle = int.MaxValue;
                foreach (Run run in Runs)
                {
                    if (run.ObstacleTimes.Count > i)
                    {
                        int time = run.ObstacleTimes[i];
                        if (time < bestAtObstacle)
                        {
                            bestAtObstacle = time;
                        }
                    }
                }
                total += bestAtObstacle;
            }
            return total;
        }
        ...
        public double ChanceOfPersonalBest(Run inProgressRun)
        {
            ...
            List<List<int>> history = GetHistory();
            int success = 0;
            for (int i = 0; i < 10000; i++)
            {
                int totalTime = 0;
                for (int j = 0; j < Course.ObstacleCount; j++)
                {
                    if (inProgressRun.ObstacleTimes.Count > j)
                    {
                        totalTime += inProgressRun.ObstacleTimes[j];
                    }
                    else
                    {
                        totalTime += GetRandomTime(history[j]);
                    }
                }
            ...
            return (double)success / 10000;
        }

        public int GetRandomTime(List<int> time)
        {
            Random rd = new Random();
            int idx = rd.Next(time.Count);
            return time[idx];
        }
