...
namespace DotNetQuestions
{
    ...
    class ImportSystem
    {
            ...
            TransactionType transactionType, int timestamp, string description, string source)
        {
            ...
            this.transactions.Add(new Transaction(transactionId, accountId, amount, transactionType, timestamp, description, source));
        ...
        public Dictionary<string, Dictionary<string, object>> GetTypeSummary()
        {
            ...
            return transactions
                .GroupBy(trs => trs.transactionType)
                .ToDictionary(
                    g => g.Key.ToString().ToUpper(),
                    g => new Dictionary<string, object>{
                        ["count"] = g.Count(),
                        ["total_amount"] = g.Sum(trs => trs.amount)
                    }
                );
        }
}
