...
## Table of Contents
1. [What does `AddCookie` register in the ASP.NET Core dependency injection containe…](#q1)
2. [What is the difference between `AddAuthentication` and `UseAuthentication`, and …](#q2)
3. [Walk through how ASP.NET Core issues an authentication cookie after a successful…](#q3)
4. [What is a `ClaimsIdentity`, what is a `ClaimsPrincipal`, and how do they relate …](#q4)
5. [How is a `ClaimsPrincipal` serialized into the authentication cookie, and how is…](#q5)
6. [What is the `AuthenticationType` property on `ClaimsIdentity` and why does it ma…](#q6)
7. [What does `Secure` mean on an authentication cookie, what risk does omitting it …](#q7)
8. [What does `HttpOnly` mean on an authentication cookie, and why is it the default…](#q8)
9. [Explain the three `SameSite` values — `Strict`, `Lax`, and `None` — and when you…](#q9)
10. [What is the difference between sliding expiration and absolute expiration on a c…](#q10)
11. [What is `IsPersistent` on `AuthenticationProperties`, how does it affect the coo…](#q11)
12. [Walk through `SignInManager.PasswordSignInAsync` end to end — every step from re…](#q12)
13. [Walk through the user registration flow in ASP.NET Core Identity — from form sub…](#q13)
14. [What validation does `UserManager.CreateAsync` perform before inserting a user r…](#q14)
15. [How does ASP.NET Core Identity handle account lockout during login, and what opt…](#q15)
16. [How does `PasswordSignInAsync` handle a user who has two-factor authentication (…](#q16)
17. [What does `SignOutAsync` do at the HTTP level, and what is required to fully des…](#q17)
18. [What is the `ReturnUrl` pattern, why is it a security risk if not validated, and…](#q18)
19. [What are anti-forgery tokens (XSRF/CSRF tokens), and how do they protect login a…](#q19)
20. [How does the ASP.NET Core Data Protection API protect the authentication cookie,…](#q20)
21. [What is `CookieAuthenticationEvents.OnValidatePrincipal`, when does it execute, …](#q21)
22. [What happens when `OnValidatePrincipal` calls `context.RejectPrincipal()`?](#q22)
23. [What claims does ASP.NET Core Identity include in the cookie by default, and how…](#q23)
24. [What is the difference between `UserManager<TUser>` and `SignInManager<TUser>` i…](#q24)
25. [What is `SignInResult` and what are its possible outcomes after calling `Passwor…](#q25)
26. [How does the `TicketDataFormat` and `IDataProtector` chain protect against cooki…](#q26)
27. [What is the relationship between the cookie authentication scheme's `LoginPath`,…](#q27)
28. [Describe three common mistakes developers make with cookie authentication in ASP…](#q28)
